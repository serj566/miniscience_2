...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = - pointZ; //pow(pointX, 2) + pow(pointY, 2) + pow(pointZ, 2);
            double vx = - 1 * (pointZ - 7) ;
            double vz = + 1 * (pointX - 12) ;
    ...
    void change_velocity(double tau){
        for(unsigned int i = 0; i < nodes.size(); i++) {
            nodes[i].vx = - 1 * (nodes[i].z - 7) ;
            nodes[i].vz = + 1 * (nodes[i].x - 12);
        }
    }
